export class EqUtils {
    ...
    static freqToX(freq: number, width: number, minFreq: number, maxFreq: number): number {
        return width * (Math.log10(freq) - Math.log10(minFreq)) /
            (Math.log10(maxFreq) - Math.log10(minFreq));
    }

    static xToFreq(x: number, width: number, minFreq: number, maxFreq: number): number {
        return Math.pow(10,
            Math.log10(minFreq) + (x / width) *
            (Math.log10(maxFreq) - Math.log10(minFreq))
        );
    }

    static dbToY(db: number, height: number, minDb: number, maxDb: number): number {
